<?php
/*
Object to control and process shortcode for wp-pubmed-reflist
*/
class wpPubMedRefList{
	...
	*/
	function wp_pubmed_reflist($key,$limit=50){
		...
		if(	$debug 
			...
			$this->options['facprops'][$key.$limit]['reflist'] = $this->reflist_query($key, $limit, $linktext);
	...
	*/
	function reflist_query($key, $limit, $linktext){
		...
		if(!empty($id_list) ){
			...
			foreach ($xml->PubmedArticle as $article){
				...
				$xrefs = $p->xrefs();
				foreach($xrefs as $key => $val){
					switch($key){
						case 'pmc':
							$linkstring .= " [<a href='http://www.ncbi.nlm.nih.gov/pmc/articles/$val'>PMC</a>]";	
							break;
						case 'doi':
							$linkstring .= " [<a href='http://dx.doi.org/$val'>Full text</a>]";
							break;
					}
				}
				$refs[] =
		...
		# to hide the link to PubMed, set Linktext to ''
		if($linktext != ''){
			$html .= "<a href=http://www.ncbi.nlm.nih.gov/pubmed?term=$query>$linktext</a>";
		}	
		return $html;
	}
